...
| --- | --- | --- | --- | --- |
| Spoofing | A device on the LAN, or DNS rebinding, reaches the port | Lodger listens on `127.0.0.1` by default. Without TLS, it refuses to start on another address, unless `trusted_proxies` or the `allow_plain_http` opt-in is set, and then it logs a warning at every start. Lodger never compares `Origin` with the `Host` header. It trusts forwarded client addresses only from `trusted_proxies`. | In place | 3.5.1, 4.1.3 |
...
| Information disclosure | Eavesdropping on the LAN | Built-in TLS, or TLS from a reverse proxy. If TLS is set, Lodger never falls back to plain HTTP. HTTPS answers send `Strict-Transport-Security`. Without TLS, Lodger refuses to start on a LAN address, unless `trusted_proxies` or the `allow_plain_http` opt-in is set. `lodger install` refuses such a configuration too. | In place | 12.1.1, 12.2.1, 3.4.1 |
